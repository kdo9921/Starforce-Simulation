var maxStarArr = [8,10,15,20,25,25,25,25];
var chance = 0;
var destroyed = 0;
var noDestCheck = 0;
var cost = 0;
...
var itemImgDB = ["100", "110", "120", "130", "140", "150", "160", "200"]

var percentDB = [   //[성공, 실패, 하락, 파괴]
    [950, 50, 0, 0], //0->1
    [900, 100, 0, 0],
    [850, 150, 0, 0],
    [850, 150, 0, 0],
...
function setInfo(){
    if ((starforce < 12) || (starforce > 16)) {
        document.getElementById("noDest").disabled = true;
    } else {
        document.getElementById("noDest").disabled = false;
    }
    ...
    totalUse += cost;
    cost = costDB[levelIndex][starforce];
    ...
    if (chance == 2) {
        ...
        if (starforce < 11 || starforce == 15 || starforce == 20){
            infoTextValue += "<br>실패(유지)확률 : " + (percentDB[starforce][1] * 0.1).toFixed(1) + "%";
        } else {
            infoTextValue += "<br>실패(하락)확률 : " + (percentDB[starforce][2] * 0.1).toFixed(1) + "%";
        }
        if (starforce > 11) {
